...
class GatekeeperAppHostImpl extends RpcTarget {
  ...
  // parent; each person the user picks is delivered to the gatekeeper server-side (see
  // AuthenticatedApi.pickGatekeeperUser) with `target`, the app's own opaque string naming what is
  // being shared. Nothing about the picked people comes back here: the promise resolves when the
  ...
  pickUsers(target: string): Promise<void> {
    if (typeof target !== 'string') return Promise.reject(new TypeError('pickUsers target must be a string.'))
